...
Galv.DETECT.event = function(id,dist,los,itemID) {
	...
	return $gameMap.event(id).distDetect(dist,los, 0, 0, itemID);
};
...
Game_Character.prototype.distDetect = function(range,los,id,balloon,itemID) {
	...
	if (itemID == 0) {
		...
		var target = id > 0 ? $gameMap.event(id) : $gameMap.event(itemID);
	}
	var x1 = this.screenX();
	var y1 = this.screenY();
	...
	if (dist <= range && !$gameSystem._undetectable) { // If in radius range of target and not undetectable
...
Game_Character.prototype.detector = function(id) {
	...
	if (detected) {
		...
		if (this._searchXY[0]) {
			...
			// 	detected = this.distDetect(Galv.DETECT.behaviors[id][2],Galv.DETECT.behaviors[id][3]);
			// 	if (detected) {
			// 		this.doDetectMove(id,1);
			// 		return;
			// 	}
			// }
			// this.doDetectMove(id,0);
		} else {
